"""Reusable validation for verification document uploads.

Mirrors the real-content-verification approach used for property image
uploads (apps/properties/serializers.py: extension + declared content type
+ size + actual file content, not just trusting metadata), extended to
cover PDF documents alongside images since verification uploads include
scanned CAC certificates and government IDs as PDFs.
"""
...
def validate_verification_document(value) -> None:
    """Validate an uploaded verification document.
    ...
    allowed_types = set(settings.VERIFICATION_DOCUMENT_ALLOWED_TYPES)
    ...
    allowed_extensions = set(settings.VERIFICATION_DOCUMENT_ALLOWED_EXTENSIONS)
    ...
    max_size = settings.VERIFICATION_DOCUMENT_MAX_SIZE_MB * 1024 * 1024
    if value.size > max_size:
        raise ValidationError(
            f"File must be {settings.VERIFICATION_DOCUMENT_MAX_SIZE_MB}MB or smaller."
        )
...
def sanitize_original_filename(filename: str) -> str:
    ...
    VerificationDocument.file's upload_to); this only sanitizes the
